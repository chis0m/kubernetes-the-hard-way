...
mkdir -p k8pki && cd k8pki
mkdir -p {ca,master,schedular,kubeproxy,controller,kubelets,admin,sa}
...
cat > master/master-kubernetes-csr.json <<EOF
{
  "CN": "kubernetes",
   ...
   "ip-10-0-1-31",
   "ip-10-0-1-32",
   "ip-10-0-1-20.${DNSName}",
   "ip-10-0-1-21.${DNSName}",
   "ip-10-0-1-22.${DNSName}",
   "ip-10-0-1-30.${DNSName}",
   "ip-10-0-1-31.${DNSName}",
   "ip-10-0-1-32.${DNSName}",
...
cat > schedular/kube-scheduler-csr.json <<EOF
{
...
cfssl gencert \
  ...
  -config=ca/ca-config.json \
  -profile=kubernetes \
  schedular/kube-scheduler-csr.json | cfssljson -bare schedular/kube-scheduler

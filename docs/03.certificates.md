...
This is to ensure encrypted communication between the server(that is, master node or pod running the kube api server), and the clients(that is, every other component).
...
Note: You can run the `pki.sh` script to create all the certificates
...
B. Using the CA create certificates for
...
1. Kube API Server/ Etcd Cert files
...
5. Kubelet
   The `Node Authorizer` mode is used to authorize api requests made from work-node's kubelets. Now, to make these kubelets "valid", you need to specify the Common Name (CN) in
   their certificates in this, I used `"CN": "system:node:${instance_hostname}"`. You will be needing this when create kubeconfig file. Generate the following
  - CSR `kubelet.csr`
...
We use a for-loop to generate this since we have 3 worker nodes. Other parts of the code are in pki.sh

```bash
# generates the cfssl json required to generate the certificates
for i in 0 1 2; do
...
The difference between a service and User Account UA is
...
for i in 0 1 2; do
  ...
  scp -i ${SSH_KEY} \
    ...
    pki/ca/ca.pem pki/kubelets/${instance}-key.pem pki/kubelets/${instance}.pem ubuntu@${external_ip}:~/; \
done
```
...
instance="${NAME}-Cluster-Master-${i}" \
  ...
  scp -i ${SSH_KEY} \
    -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
    pki/ca/ca.pem pki/ca/ca-key.pem \
    pki/sa/service-account.pem pki/sa/service-account-key.pem \
    pki/master/master-kubernetes.pem pki/master/master-kubernetes-key.pem ubuntu@${external_ip}:~/;
done
```
